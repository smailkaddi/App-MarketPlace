...
function LoginSuperAdmin() {
    ...
    const idDelevry=localStorage.getItem('idDelivry');
    let idDelivry;
    ...
    useEffect(()=>{
        ...
        axios.get(`http://localhost:8080/Delivry/${idDelevry}`)
        .then(function (response) {
         
        setDelivry_Name(response.data.Delivry_Name)
    ...
    const handleSubmit = (e) => {
      ...
      const Delivrydata = {Delivry_Name,Type:id};
          axios.put(`http://localhost:8080/Delivry/update/${idDelevry}`,Delivrydata)
            .then(res => {
              if(res.error){
              return false
          }else{
    ...
    <div className="flex flex-col justify-center md:justify-start my-auto pt-8 md:pt-0 px-8 md:px-24 lg:px-32">
      ...
      <form className="flex flex-col pt-3 md:pt-8" onSubmit={handleSubmit}>
        <div className="flex flex-col pt-4">
          ...
          <input type="text" id="text" placeholder="yassine" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mt-1 leading-tight focus:outline-none focus:shadow-outline" 
           value={Delivry_Name}
